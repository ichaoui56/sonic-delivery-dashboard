...
import { Package, Search, Filter, Warehouse } from "lucide-react"
type Product = {
  id: number
  name: string
  image: string | null
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-2 sm:p-4 md:p-6">
      <div className="mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-2">مخزون المستودع</h1>
        <p className="text-xs sm:text-sm text-gray-600">عرض وإدارة جميع المنتجات المتوفرة في المستودع</p>
      </div>
                    ...
                    <Card key={product.id} className="p-2 sm:p-3 hover:shadow-md transition-shadow">
                      <div className="flex gap-2 sm:gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {product.sku && (
                            <p className="text-[10px] sm:text-xs text-gray-600 mb-1">SKU: {product.sku}</p>
                          )}
                          <div className="space-y-0.5 text-[10px] sm:text-xs">
                            <div className="flex items-center justify-between">
                              <span className="text-gray-600">المخزون:</span>
                              <span className="font-bold text-[#048dba]">
                                {product.stockQuantity.toLocaleString("en-US")} قطعة
                              </span>
                            </div>
                            {lastTransfer && (
